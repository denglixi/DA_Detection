...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None, seg_return=False, path_return=False,
        ...
        self.transform4ts = transform4ts
        if self.transform4ts:
            self.transforms = transforms.Compose([transforms.RandomCrop(600)])
    ...
    def __getitem__(self, index):
        ...
        blobs = get_minibatch(
            minibatch_db, self._num_classes, seg_return=self.seg_return,
            transform4ts=self.transform4ts)
        ...
        if self.training and data.shape[0] == 1:
            ...
            if self.seg_return:
                seg_map = torch.from_numpy(
                    np.resize(blobs['seg_map'], (data_height, data_width)))
            ...
            image_channel = data.shape[3]
            batch_size = data.shape[0]
            data = data.permute(0, 3, 1, 2).contiguous().view(
                image_channel * batch_size, data_height, data_width)
            im_info = im_info.view(3)

...
if __name__ == '__main__':
    ...
    elif args.net == 'res101_weakly':
        fasterRCNN = resnet_weakly(imdb.classes, 101, pretrained=True,
                                   class_agnostic=args.class_agnostic,
                                   lc=args.lc, gc=args.gc)

    elif args.net == 'res50_weakly':
        fasterRCNN = resnet_weakly(imdb.classes, 50, pretrained=True,
                                   class_agnostic=args.class_agnostic,
                                   lc=args.lc, gc=args.gc)
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            rois, cls_prob, bbox_pred, \
                ...
                rois_label, out_d_pixel, out_d = fasterRCNN(
            ...
            if step % args.disp_interval == 0:
                ...
                if args.train_domain_loss:
                    print(
                            "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f eta: %.4f bce loss: %.4f"
